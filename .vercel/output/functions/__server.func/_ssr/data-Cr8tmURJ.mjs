...
var DELIVERABLES = [
	{
		id: "note-h302",
		name: "Approval_Note_H-302.doc",
		kind: "note",
		size: "18 KB",
		from: "Inspection demo"
	},
	{
		id: "sheet-h302",
		name: "H-302_findings.csv",
		kind: "sheet",
		size: "2 KB",
		from: "Inspection demo"
	},
...
var SCENARIOS = [
	{
		...
		prompt: "Read the scanned inspection report for fired heater H-302 from the April turnaround. Extract findings, check against OISD-STD-116, and draft an approval note for the plant manager.",
		events: [
			{
				at: 200,
				kind: "user",
				text: "Read the scanned inspection report for fired heater H-302 from the April turnaround. Extract findings, check against OISD-STD-116, and draft an approval note for the plant manager."
			},
			{
				at: 700,
				kind: "route",
				model: "qwen-vl",
				reason: "Task starts with a scanned PDF and handwritten margin notes. Vision first."
			},
			{
				at: 1100,
				kind: "plan",
				items: [
					"OCR the H-302 scan",
					"Ground findings in OISD-STD-116",
					"Draft plant-manager approval note",
					"Write Word + findings sheet"
				]
			},
			{
				at: 1600,
				kind: "tool",
				name: "vision.ocr",
				detail: "H-302_inspection_scan.pdf · 3 pages · handwriting layer",
				status: "run"
			},
			{
				at: 2800,
				kind: "tool",
				name: "vision.ocr",
				detail: "Extracted 14 findings · confidence 0.91",
				status: "ok"
			},
			{
				at: 3200,
				kind: "route",
				model: "llama-reason",
				reason: "Long-form note and policy language. Switching to Llama 3.3 70B."
			},
			{
				at: 3600,
				kind: "tool",
				name: "kb.search",
				detail: "OISD-STD-116 fired heater flame envelope",
				status: "run"
			},
			{
				at: 4400,
				kind: "tool",
				name: "kb.search",
				detail: "3 passages · vault hit, no external fetch",
				status: "ok"
			},
			{
				at: 5e3,
				kind: "assistant",
				text: "H-302 is not in a fit state for full-rate restart. Three items sit outside the firebox envelope in OISD-STD-116: tube-skin +48°C on pass 3, refractory spall at peephole 4, and burner 2 air register locked at 30%. I recommend derate 8%, patch refractory this window, and fabricate an interim register sleeve (site shop, 9 days) while the OEM part is on 14-week lead. Approval note and findings sheet are on the bench."
			},
			{
				at: 5600,
				kind: "artifact",
	...
	},
	{
		...
		prompt: "Write a Python function for shell-and-tube duty and LMTD from the Unit 3 process data. Run tests in the sandbox. Do not call any network.",
		events: [
			{
				at: 200,
				kind: "user",
				text: "Write a Python function for shell-and-tube duty and LMTD from the Unit 3 process data. Run tests in the sandbox. Do not call any network."
			},
			{
				at: 700,
				kind: "route",
				model: "qwen-coder",
				reason: "Code generation and unit tests. Coder 32B selected over the general instruct model."
			},
			{
				at: 1100,
				kind: "plan",
				items: [
					"Read process data from vault",
					"Implement Q and LMTD",
					"Add Ft correction",
					"Run pytest in Firecracker"
				]
			},
			{
				at: 1500,
				kind: "tool",
				name: "files.read",
				detail: "vault://hex-manual §4.2 process data",
				status: "ok"
			},
			{
				at: 2e3,
				kind: "tool",
				name: "sandbox.exec",
				detail: "python -m pytest exchanger_duty_test.py -q",
				status: "run"
			},
			{
				at: 3400,
				kind: "tool",
				name: "sandbox.exec",
				detail: "4 passed in 0.08s · egress syscalls: 0",
				status: "ok"
			},
			{
				at: 4e3,
				kind: "assistant",
				text: "Duty for the given train is 4.82 MW. LMTD is 18.4 °C with Ft = 0.93 on the two-pass shell. Arithmetic mean would have overstated driving force by 11% — the sandbox test locks that in. Script is on the bench; nothing left the jail."
			},
			{
				at: 4600,
				kind: "artifact",
	...
	},
	{
		...
		prompt: "On this P&ID extract, identify isolation for pump P-101A. List blinds required and cite the Unit 3 isolation SOP.",
		events: [
			{
				at: 200,
				kind: "user",
				text: "On this P&ID extract, identify isolation for pump P-101A. List blinds required and cite the Unit 3 isolation SOP."
			},
			{
				at: 700,
				kind: "route",
				model: "qwen-vl",
				reason: "Engineering drawing. Vision model with P&ID legend overlay."
			},
			{
				at: 1100,
				kind: "plan",
				items: [
					"Read drawing symbols",
					"Identify P-101A envelope",
					"Match isolation SOP",
					"Emit tagged list"
				]
			},
			{
				at: 1600,
				kind: "tool",
				name: "vision.read_drawing",
				detail: "P101A_extract.png · ISO 10628 overlay",
				status: "run"
			},
			{
				at: 2600,
				kind: "tool",
				name: "vision.read_drawing",
				detail: "Suction GV-101A, discharge GV-102A, drain DV-14",
				status: "ok"
			},
			{
				at: 3e3,
				kind: "route",
				model: "llama-reason",
				reason: "SOP citation and permit language."
			},
			{
				at: 3400,
				kind: "tool",
				name: "kb.search",
				detail: "Turnaround Isolation SOP — P-101A",
				status: "ok"
			},
			{
				at: 4100,
...
var APPROVAL_NOTE_HTML = `<html><head><meta charset="utf-8"><title>Approval Note H-302</title>
...
var FINDINGS_CSV = `item,location,deviation,standard,action,window
...
var EXCHANGER_PY = `"""Shell-and-tube duty and LMTD. Local only."""
...
`;
var SLIDES_HTML = `<html><head><meta charset="utf-8"><title>Turnaround brief</title>
...
//#endregion
export { LOCAL_NET as a, SEED_PACKETS as c, modelById as d, FINDINGS_CSV as i, SLIDES_HTML as l, DELIVERABLES as n, MODELS as o, EXCHANGER_PY as r, SCENARIOS as s, APPROVAL_NOTE_HTML as t, VAULT as u };
